...
function updateMap(variable) {
  ...
  geojsonLayer = L.geoJSON(null, {
    style: feature => styleFeature(feature, variable, primaryOpacity),
...
function updateOverlay(variable) {
  ...
  overlayLayer = L.geoJSON(null, {
    style: feature => styleFeature(feature, variable, overlayOpacity)
  });
  fetch(geojsonUrl)
    .then(res => res.json())
    .then(data => overlayLayer.addData(data).addTo(map));
}
...
document.getElementById('overlay-opacity-slider').addEventListener('input', (e) => {
  ...
  updateOverlay(document.getElementById('overlay-variable').value);
});
